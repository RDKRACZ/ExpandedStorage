...
public class StorageMutator extends Item {
                ...
                if (state.contains(Properties.CHEST_TYPE)) {
                    ...
                    if (chestType != ChestType.SINGLE) {
                        if (!world.isClient()) {
                            ...
                            world.setBlockState(pos, state.rotate(BlockRotation.CLOCKWISE_180).with(Properties.CHEST_TYPE, state.get(Properties.CHEST_TYPE).getOpposite()));
                            world.setBlockState(otherPos, otherState.rotate(BlockRotation.CLOCKWISE_180).with(Properties.CHEST_TYPE, otherState.get(Properties.CHEST_TYPE).getOpposite()));
                        }
        ...
        if (block instanceof net.minecraft.block.ChestBlock) {
                ...
                if (tag.contains("pos")) {
                    ...
                    if (otherState.getBlock() == state.getBlock() &&
                        ...
                        if (!world.isClient()) {
                            ...
                            if (direction != null) {
                                CursedChestType type = AbstractChestBlock.getChestType(state.get(Properties.HORIZONTAL_FACING), direction);
                                Predicate<BlockEntity> isLootable = b -> b instanceof LootableContainerBlockEntity;
                                this.convertBlock(world, state, pos, Common.getTieredBlock(Common.CHEST_BLOCK_TYPE, Utils.WOOD_TIER_ID), Utils.WOOD_STACK_COUNT, type, isLootable);
                                this.convertBlock(world, otherState, otherPos, Common.getTieredBlock(Common.CHEST_BLOCK_TYPE, Utils.WOOD_TIER_ID), Utils.WOOD_STACK_COUNT, type.getOpposite(), isLootable);
                                tag.remove("pos");
                                //noinspection ConstantConditions
            ...
            if (StorageMutator.getMode(stack) == MutationMode.ROTATE) {
                if (!world.isClient()) {
                    ...
                    world.setBlockState(pos, state.with(Properties.FACING, Direction.byId(direction.getId() + 1)));
                }
                //noinspection ConstantConditions
                player.getItemCooldownManager().set(this, Utils.QUARTER_SECOND);
                return ActionResult.SUCCESS;
    ...
    private void convertBlock(World world, BlockState state, BlockPos pos, Block block, int slotCount, @Nullable CursedChestType type, Predicate<BlockEntity> check) {
        ...
        if (check.test(targetBlockEntity)) {
            ...
            //noinspection ConstantConditions
            Inventories.readNbt(targetBlockEntity.createNbt(), invData);
            ...
            if (state.contains(Properties.FACING)) {
                newState = newState.with(Properties.FACING, state.get(Properties.FACING));
            ...
            if (type != null) {
                newState = newState.with(AbstractChestBlock.CURSED_CHEST_TYPE, type);
            }
            world.setBlockState(pos, newState);
            BlockEntity newEntity = world.getBlockEntity(pos);
            //noinspection ConstantConditions
            newEntity.readNbt(Inventories.writeNbt(newEntity.createNbt(), invData));
        }
    }
    ...
    protected ActionResult useModifierOnBlock(ItemUsageContext context, BlockState state, BlockPos pos, @SuppressWarnings("unused") Type type) {
        ...
        if (mode == MutationMode.MERGE) {
            if (block instanceof AbstractChestBlock chestBlock && state.get(AbstractChestBlock.CURSED_CHEST_TYPE) == CursedChestType.SINGLE) {
                NbtCompound tag = stack.getOrCreateNbt();
                if (tag.contains("pos")) {
                    BlockPos otherPos = NbtHelper.toBlockPos(tag.getCompound("pos"));
                    BlockState otherState = world.getBlockState(otherPos);
                    ...
                    if (block == otherState.getBlock()
                        ...
                        if (!world.isClient()) {
                            ...
                            if (direction != null) {
                                ...
                                Predicate<BlockEntity> isStorage = b -> b instanceof OpenableBlockEntity;
